...
export default function Demo(){
  ...
  return <main>
    ...
    <section className="hero" style={{gridTemplateColumns:"1fr",paddingBottom:32}}><div>
      ...
      <p className="lead">Choose an AI-agent action and run it through SENTRYA's live policy engine. Public evaluations are read-only.</p>
    </div></section>
    ...
    <section style={{paddingTop:32}}><h2>Human Approval Center</h2><p className="lead">Sensitive agent actions stop here until a human decides.</p>{approvals.length===0?<p>No persisted approval requests yet. Authenticated admin simulations appear here.</p>:<div className="grid">{approvals.slice(0,6).map(a=><article key={a.id}><small>{a.risk_level} RISK</small><h3>{a.action}</h3><b className={a.status==="PENDING"?"bad":"good"}>{a.status}</b><p>{a.resource??"No resource"}</p></article>)}</div>}</section>
