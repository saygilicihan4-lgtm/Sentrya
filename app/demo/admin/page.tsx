...
export default function AdminDemo() {
  ...
  return <main><nav><a href="/" style={{color:"inherit",textDecoration:"none"}}><b>SENTRYA</b></a><span>ADMIN CONTROL PLANE</span></nav>
    ...
    <section><div className="terminal"><div className="dots">● ● ● <span>OPERATOR AUTH</span></div><div style={{padding:"22px"}}>
      ...
      :<div><b className="good">ADMIN SESSION ACTIVE</b><p>Human approval and emergency controls are unlocked.</p><button onClick={refresh} disabled={busy}>Refresh</button> <button onClick={checkSchema} disabled={busy}>Verify database</button> <button onClick={checkGitHub} disabled={busy}>Check GitHub App</button> <button onClick={seed} disabled={busy}>Enroll demo agent</button> <button onClick={simulate} disabled={busy}>Simulate protected action</button> <button onClick={logout}>Sign out</button>{schema&&<p><b>{schema}</b></p>}{github&&<p><b>{github}</b></p>}</div>}{message&&<p>{message}</p>}
    </div></div></section>
