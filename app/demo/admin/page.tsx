...
export default function AdminDemo() {
  ...
  const [agents,setAgents]=useState<Agent[]>([]); const [approvals,setApprovals]=useState<Approval[]>([]); const [audit,setAudit]=useState<AuditEvent[]>([]); const [schema,setSchema]=useState<string>(""); const [github,setGithub]=useState<string>("");
  ...
  async function refresh(){
    const [a,p,l]=await Promise.all([
      ...
      fetch("/api/v1/audit").then(r=>r.json() as Promise<{events?:AuditEvent[]}>)
    ]);
    setAgents(a.agents??[]); setApprovals(p.approvals??[]); setAudit(l.events??[]);
  }
  ...
  async function checkGitHub(){const r=await fetch("/api/v1/integrations/github/status");const d=await r.json() as {configured?:boolean;error?:string};setGithub(r.ok?(d.configured?"GitHub App: CONFIGURED":"GitHub App: NOT CONFIGURED"):"GitHub App: "+(d.error??"CHECK FAILED"));}
  async function probeGitHub(){setBusy(true);const r=await fetch("/api/v1/integrations/github/repository");const d=await r.json() as {connected?:boolean;repository?:string;error?:string;githubStatus?:number};setGithub(r.ok&&d.connected?`GitHub connection: VERIFIED • ${d.repository??"repository"}`:`GitHub connection: ${d.error??"FAILED"}${d.githubStatus?" ("+d.githubStatus+")":""}`);setBusy(false);}
  async function logout(){await fetch("/api/v1/admin/login",{method:"DELETE"});setLogged(false);setAgents([]);setApprovals([]);setAudit([]);setMessage("Session closed.");}
  ...
  async function simulate(){setBusy(true);const r=await fetch("/api/v1/evaluate",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({agentId:"demo-agent",action:"gmail.send_email",resource:"demo/outbound-message"})});const d=await r.json() as {decision?:string;persisted?:boolean;error?:string};setMessage(r.ok?`Simulation: ${d.decision??"evaluated"} • ${d.persisted?"persisted":"not persisted"}`:(d.error??"Simulation failed."));await refresh();setBusy(false);}
  ...
  return <main><nav><a href="/" style={{color:"inherit",textDecoration:"none"}}><b>SENTRYA</b></a><span>ADMIN CONTROL PLANE</span></nav>
    ...
    <section><div className="terminal"><div className="dots">● ● ● <span>OPERATOR AUTH</span></div><div style={{padding:"22px"}}>
      ...
      :<div><b className="good">ADMIN SESSION ACTIVE</b><p>Human approval and emergency controls are unlocked.</p><button onClick={refresh} disabled={busy}>Refresh</button> <button onClick={checkSchema} disabled={busy}>Verify database</button> <button onClick={checkGitHub} disabled={busy}>Check GitHub App</button> <button onClick={probeGitHub} disabled={busy}>Verify GitHub connection</button> <button onClick={seed} disabled={busy}>Enroll demo agent</button> <button onClick={simulate} disabled={busy}>Simulate protected action</button> <button onClick={logout}>Sign out</button>{schema&&<p><b>{schema}</b></p>}{github&&<p><b>{github}</b></p>}</div>}{message&&<p>{message}</p>}
    </div></div></section>
    ...
    <section style={{paddingTop:32}}><h2>Audit Ledger</h2>{audit.length===0?<p>No persisted audit events yet.</p>:<div className="grid">{audit.slice(0,8).map(e=><article key={e.id}><small>{e.risk_level} RISK</small><h3>{e.action}</h3><b>{e.decision}</b><p>Hash: {e.event_hash?.slice(0,16)}…</p></article>)}</div>}</section></>}
