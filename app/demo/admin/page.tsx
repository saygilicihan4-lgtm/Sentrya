...
export default function AdminDemo() {
  ...
  const [agents,setAgents]=useState<Agent[]>([]); const [approvals,setApprovals]=useState<Approval[]>([]);
  ...
  return <main><nav><a href="/" style={{color:"inherit",textDecoration:"none"}}><b>SENTRYA</b></a><span>ADMIN CONTROL PLANE</span></nav>
    ...
    <section><div className="terminal"><div className="dots">● ● ● <span>OPERATOR AUTH</span></div><div style={{padding:"22px"}}>
      ...
      :<div><b className="good">ADMIN SESSION ACTIVE</b><p>Human approval and emergency controls are unlocked.</p><button onClick={refresh} disabled={busy}>Refresh</button> <button onClick={seed} disabled={busy}>Enroll demo agent</button> <button onClick={logout}>Sign out</button></div>}{message&&<p>{message}</p>}
    </div></div></section>
